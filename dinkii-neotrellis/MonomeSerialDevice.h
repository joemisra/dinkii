...
public:
    ...
    uint8_t getRed() { return ledR; }
    uint8_t getGreen() { return ledG; }
    uint8_t getBlue() { return ledB; }
    uint8_t getBrightness() { return globalBrightness; }
...
private:
    ...
    // RGB values for colored LEDs
    uint8_t ledR = 255;
    uint8_t ledG = 255;
    uint8_t ledB = 255;
    uint8_t globalBrightness = 32; // Default brightness (0-255)

    //        MonomeSerialDevice();
    void processSerial();
    void handleSystemCommand(uint8_t cmd);
    void handleLedGridCommand(uint8_t cmd);
    void handleKeyGridCommand(uint8_t cmd);
    void handleEncoderCommand(uint8_t cmd);
    void handleTiltCommand(uint8_t cmd);
    void handleLedRingCommand(uint8_t cmd);
    void handleRgbCommand(uint8_t cmd); // New handler for RGB commands
};
